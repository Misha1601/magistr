...
def main():
    ...
    # Начальные значения параметров
    popt, pcov = curve_fit(Gompertz1, (data.data0[1:], indexdata[1:]), data.generate[1:], bounds=(0, np.inf), method='trf', maxfev = 10000)
    b0 = popt[0]
    c0 = popt[1]
    m0 = popt[2]
    ...
    for n in metod_list:
        ...
        try:
            res = minimize(squareMistakeGompertz1, k0, args=generate2, method=n, bounds=kb)
        except:
        ...
        while years2[-1] < finalYear:
            ...
            prSales.append(Gompertz1((generate[0], prCumul), k[0], k[1], k[2]))  # Добавляем следующий Prognose
        ...
        square = squareMistakeGompertz1(k, generate)

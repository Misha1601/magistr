...
def main():
    ...
    # Начальные значения параметров
    z = np.array([data0_values, indexdata, total_values, costs_values])
    ...
    costs2 = np.array(data.costs)

    for n in metod_list:
        # Минимизируем сумму квадратов
        try:
            res = minimize(squareMistakeGompertz3, k0, args=(generate2, total2, costs2), method=n, bounds=kb)
        except:
        ...
        while years2[-1] < finalYear:
            ...
            z = generate[0], ind_tgen, m[ind_tgen - 1], c[ind_tgen - 1]   # indexdata, total_values
        ...
        square = squareMistakeGompertz3(k, generate, total, costs)

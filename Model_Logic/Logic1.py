...
def main():
    ...
    def squareMistake(k: tuple, *sales) -> float:
        ...
        for i in range(1, len(sales)):
            p = Logic(i, B=k[0], C=k[1], M=k[2])  # Новый Prognose Sales
            res += (p - sales[i])**2  # Добавляем
        return res

    # Начальные значения параметров
    popt, pcov = curve_fit(Logic, indexdata[1:], data.generate[1:], bounds=(0, np.inf), method='trf', maxfev = 10000)
    b0 = popt[0]
    c0 = popt[1]
    m0 = popt[2]
    ...
    for n in metod_list:
        ...
        while years2[-1] < finalYear:
            ...
            prSales.append(Logic(prCumul, B=k[0], C=k[1], M=k[2]))  # Добавляем следующий Prognose
